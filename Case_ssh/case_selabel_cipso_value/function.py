...
class Test_selabel_cipso_value():
    ...
    def setup_class(self):
        ...
        self.read_6 = self.pkt6_cfg["read"][0]
        self.read_7 = self.pkt7_cfg["read"][0]
        self.read_8 = self.pkt8_cfg["read"][0]
    ...
    def test_selabel_category_cipso_value_range4(self):
        ...
        # 检查报文是否存在
        # fun.wait_data(f"ls /opt/pkt/ | grep pcap", 'gw', self.pkt6_cfg['capture'][3], '检查服务端抓包结果', 300, flag='存在')
        pcap_file = fun.search('/opt/pkt', 'pcap', 's')
        assert cap_pcap in pcap_file
        print('服务端抓到报文：{}'.format(self.read_6))
    ...
    def test_selabel_category_cipso_value_range5(self):
        ...
        # 检查报文是否存在
        # fun.wait_data(f"ls /opt/pkt/ | grep pcap", 'gw', self.pkt7_cfg['capture'][3], '检查服务端抓包结果', 300, flag='存在')
        pcap_file = fun.search('/opt/pkt', 'pcap', 's')
        assert cap_pcap in pcap_file
        print('服务端抓到报文：{}'.format(self.read_7))
    ...
    def test_selabel_category_cipso_value_range6(self):
        ...
        # 检查报文是否存在
        # fun.wait_data(f"ls /opt/pkt/ | grep pcap", 'gw', self.pkt8_cfg['capture'][3], '检查服务端抓包结果', 300, flag='存在')
        pcap_file = fun.search('/opt/pkt', 'pcap', 's')
        assert cap_pcap in pcap_file
        print('服务端抓到报文：{}'.format(self.read_8))

...
print(base_path)
try:
	from iso_mail_check_subject import index
	from iso_mail_check_subject import message
...
class Test_iso_mail_check_subject():
	...
	def test_iso_mail_check_subject_a1(self):
		...
		print('2、下发黑名单主题：test，等待nginx的24个进程起来；cat /etc/jsac/filter.json文件应该包含：deny-topic和黑名单主题：test')
		...
		# 发送邮件,邮件地址为非黑名单主题
		print('4、控制台发送邮件，邮件主题为非黑名单主题，查看发送结果；发送成功')
		...
		# 接收邮件
		print('5、接收邮件，查看pop3协议收到的邮件主题是否为非黑名单主题；接收邮件主题为非黑名单主题')
		msg = recv_pop3.get_email(self.pop3_email, self.pop3_pwd, proxy_ip, self.pop3_server_port)
	...
	def test_iso_mail_check_subject_a2(self):
		...
		print('2、下发黑名单主题：test和abc，等待nginx的24个进程起来；cat /etc/jsac/filter.json文件应该包含：deny-topic和黑名单主题：test、abc')

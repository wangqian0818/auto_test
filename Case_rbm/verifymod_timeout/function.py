...
6.用命令ipauth-jsac --auth --show在老化时间内持续查询认证ip表
7.移除根证书文件
...
2.根证书下发成功，ls /usr/local/ipauth/file/CAdb/可以查看到根证书md5值c970dfa3234ed84ccac2e819a1276026
3.使用命令cat /etc/jsac/Initialize.conf查询到AUTHIP_DURATION = 5说明认证时效设置成功
4.认证请求返回结果verify success说明请求发送成功
5.日志查询到"jsacaudit 源ip success"，认证ip表查询到源ip说明认证成功
6.认证ip表查询不到源ip说明ip已老化
7.根证书移除成功，ls /usr/local/ipauth/file/CAdb/无法查到证书md5值c970dfa3234ed84ccac2e819a1276026
8.认证服务关闭成功，查询不到服务进程/usr/local/ipauth/verifymod /etc/jsac/Initialize.conf、且认证服务端口443也不存在

脚本二
用例名称：验证认证服务的认证时效设置功能
编写人员：马丹丹
编写日期：2021/7/21
测试目的：验证认证服务的认证时效设置功能
测试步骤：
1.开启认证服务，通过命令ps -ef | grep verifymod、netstat -ultpn查询服务进程及服务端口是否存在
2.设置认证时效为120s
...
预期结果：
1.认证服务开启成功，服务进程有/usr/local/ipauth/verifymod /etc/jsac/Initialize.conf、服务有监听443端口说明服务开启成功
2.使用命令cat /etc/jsac/Initialize.conf查询到AUTHIP_DURATION = 120说明认证时效设置成功
3.认证服务关闭成功，查询不到服务进程/usr/local/ipauth/verifymod /etc/jsac/Initialize.conf、且认证服务端口443也不存在
...
print(base_path)
try:
	from verifymod_timeout import index
	from verifymod_timeout import message
...
class Test_verifymod_timeout():
    ...
    @allure.feature('验证认证ip老化功能')
    def test_verifymod_ip_timeout(self):
        ...
        print('3.设置认证时效为5s，使用命令cat /etc/jsac/Initialize.conf查询到AUTHIP_DURATION = 5说明认证时效设置成功')
        fun.send(rbmExc, message.verifymod_Timeout['ConfigAuthServer'], domain_rmb, base_path)
        fun.wait_data(self.case1_step4["step1"][0], 'gw', self.case1_step4["step1"][1], '检查认证时效', 100)
        for key in self.case1_step4:
            re1 = fun.cmd(self.case1_step4[key][0],'gw')
        ...
        print('发送认证url：',self.case1_step5["step1"][0])
        for key in self.case1_step5:
            re1 = fun.cmd(self.case1_step5[key][0],'c')
            print(re1)
        ...
        fun.wait_data(self.case1_step6["step1"][0],'gw', self.case1_step6["step1"][1], '检查认证日志', 100)
        for key in self.case1_step6:
            re1 = fun.cmd(self.case1_step6[key][0],'gw')
            assert self.case1_step6[key][1] in re1
        fun.wait_data(self.case1_step7["step1"][0], 'gw', self.case1_step7["step1"][1], '检查认证ip', 100)
        re1 = fun.cmd(self.case1_step7["step1"][0], 'gw')
        assert self.case1_step7["step1"][1] in re1

        print('6.用命令ipauth-jsac --auth --show在老化时间内持续查询认证ip表，认证ip表查询不到源ip说明ip已老化')
        fun.wait_data(self.case1_step7["step1"][0], 'gw', self.case1_step7["step1"][1], '检查认证ip', 600, flag='不存在')
        re1 = fun.cmd(self.case1_step7["step1"][0], 'gw')
        assert self.case1_step7["step1"][1] not in re1

        print('7.移除根证书文件，根证书移除成功，ls /usr/local/ipauth/file/CAdb/无法查到证书md5值c970dfa3234ed84ccac2e819a1276026')
        fun.send(rbmExc, message.verifymod_DelAuthCert['DelAuthCert'], domain_rmb, base_path)
        # 检查配置下发是否成功
        for key in self.case1_step3:
        ...
        # 关闭认证服务
        print('8.关闭认证服务，通过命令ps -ef | grep verifymod、netstat -ultpn查询不到服务进程/usr/local/ipauth/verifymod /etc/jsac/Initialize.conf、且认证服务端口443也不存在')
    ...
    # @pytest.mark.skip(reseason="skip")
    @allure.feature(' 验证认证服务的认证时效设置功能')

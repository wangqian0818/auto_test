...
print(base_path)
try:
    from iso_mail_check_file import index
    from iso_mail_check_file import message
...
class Test_iso_mail_check_file():
    ...
    def test_iso_mail_check_file_a1(self):
        ...
        print('2、下发黑名单附件名：test，等待nginx的24个进程起来；cat /etc/jsac/filter.json文件应该包含：deny-basename和黑名单主题：test')
        ...
        # 发送邮件,邮件地址为黑名单文件名
        print('3、控制台发送邮件，邮件附件文件名为黑名单附件名：test，查看发送结果；发送失败')
        ...
        # 发送邮件,邮件地址为非黑名单文件名
        print('4、控制台发送邮件，邮件附件文件名为非黑名单附件名：123，查看发送结果；发送成功')
        ...
        # 接收邮件
        print('5、接收邮件，查看pop3协议收到的邮件内容是否为非黑名单文件名发送的；接收邮件内容为非黑名单文件名发送的')
    ...
    def test_iso_mail_check_file_a2(self):
        ...
        print('2、下发黑名单附件名：test、juson，等待nginx的24个进程起来；cat /etc/jsac/filter.json文件应该包含：deny-basename和黑名单附件名：test、juson')
        ...
        print('4、控制台发送邮件，邮件附件文件名为黑名单附件名：juson，查看发送结果；发送失败')
        result2 = send_smtp.post_email(self.mail_sender, self.mail_receivers, self.mail_cc, self.mail_bcc,
                                       self.mail_host, self.mail_port, self.mail_user, self.mail_pass,
        ...
        # 发送邮件,邮件地址为非黑名单文件名
        print('5、控制台发送邮件，邮件附件文件名为非黑名单附件名：123，查看发送结果；发送成功')
        ...
        # 接收邮件
        print('6、接收邮件，查看pop3协议收到的邮件内容是否为非黑名单文件名发送的；接收邮件内容为非黑名单文件名发送的')
        msg = recv_pop3.get_email(self.pop3_email, self.pop3_pwd, self.pop3_server_host, self.pop3_server_port)
        mail_list = recv_pop3.print_info(msg)  # 解析

...
2、cat /etc/jsac/http.json文件应该包含：c_http_uri和uri黑名单参数：123
...
脚本二：
用例名称：验证隔离下基于uri方法的多个网页访问策略的过滤
编写人员：李皖秋
编写日期：2021.7.7
测试目的：验证隔离下基于uri方法的多个网页访问策略的过滤
测试步骤：
1、下发http的隔离代理：代理ip为前置机安全卡的ip，port为2287，等待nginx的24个进程起来
2、下发http的uri黑名单，参数为：123，等待nginx的24个进程起来
3、控制台发送get请求，不包含请求内容
...
6、控制台发送post请求，请求内容包含黑名单test
7、控制台发送get请求，请求内容包含黑名单juson
8、控制台发送post请求，请求内容包含黑名单juson
9、移除http的隔离策略，清空环境，等待nginx的24个进程起来
10、移除网页访问策略，等待nginx的24个进程起来
预期结果：
1、cat /etc/jsac/http.stream应该包含代理ip和port，且netstat -anp |grep tcp应该可以查看到监听ip和端口
2、cat /etc/jsac/http.json文件应该包含：c_http_uri和uri黑名单参数：test、juson
3、请求成功，请求到的内容为server的index.html文件内包含的内容
4、请求成功，请求到的内容为server的index.html文件内包含的内容
5、请求失败，状态码返回为403
6、请求失败，状态码返回为403
7、请求失败，状态码返回为403
8、请求失败，状态码返回为403
9、cat /etc/jsac/http.stream应该不包含代理ip和port
10、cat /etc/jsac/http.json文件应该不包含：http协议
'''
...
print(base_path)
try:
	from iso_http_check_uri import index
	from iso_http_check_uri import message
...
class Test_iso_http_check_uri():
	...
	def test_iso_http_check_uri_a1(self):
		...
		# 数据检查
		print('2、下发http的uri黑名单，参数为：123，等待nginx的24个进程起来;预期cat /etc/jsac/http.json文件应该包含：c_http_uri和uri黑名单参数：123')
		...
		# 发送post请求，请求内容包含黑名单
		print('6、控制台发送post请求，请求内容包含黑名单;请求失败，状态码返回为403')
		...
		# 检查网页访问策略是否清空
		print('8、移除网页访问策略，等待nginx的24个进程起来;cat /etc/jsac/http.json文件应该不包含：c_http_uri和uri黑名单参数：123')

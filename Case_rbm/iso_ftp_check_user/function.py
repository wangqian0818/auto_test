...
print(base_path)
try:
    from iso_ftp_check_user import index
    from iso_ftp_check_user import message
...
class Test_iso_ftp_check_user():
    ...
    def test_iso_ftp_check_user_a1(self):
        ...
        print('2、下发ftp的用户白名单：test，等待nginx的24个进程起来；cat /etc/jsac/filter.json文件应该包含：allow-user和用户白名单：test')
        ...
        print('4、控制台走ftp隔离登录ftp服务器，用户为非白名单lwq，查看登录是否成功；登录失败')
        # 登录ftp服务器，用户为非白名单用户

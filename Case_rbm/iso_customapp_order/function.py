...
try:
	...
	from iso_customapp_order import message
...
class Test_customapp_order():
    ...
    @allure.feature('验证过滤类型为命令时，测试配置命令是否区分大小写的定制应用策略---命令大写')
    def test_customapp_order_capital(self):

        # 下发配置
        fun.send(rbmExc, message.add_app_order_capital_front['AddCustomAppPolicy'], FrontDomain, base_path)
        fun.send(rbmExc, message.add_app_order_capital_back['AddCustomAppPolicy'], BackDomain, base_path)
        ...
        print('url:', url)
        print('定制应用命令为大写时,get请求的请求内容为：{}'.format(status_code))
        assert status_code == 200

        # 文件查看audit日志
        fun.wait_data(f"grep -n {proxy_port} /usr/local/nginx/logs/audit.log |tail -2", 'FrontDut',self.case1_step3['step1'][0], '检查大写字母cmdword GET', 300, flag='存在')
    ...
    # @pytest.mark.skip(reseason="skip")
    @allure.feature('验证过滤类型为命令时，测试配置命令是否区分大小写的定制应用策略---命令小写')
    def test_customapp_order_lowercase(self):

        # 下发配置
        fun.send(rbmExc, message.add_app_order_lowercase_front['AddCustomAppPolicy'], FrontDomain, base_path)
        fun.send(rbmExc, message.add_app_order_lowercase_back['AddCustomAppPolicy'], BackDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        front_res1 = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process', name='前置机nginx进程')
        assert front_res1 == 1
        fun.wait_data('ps -ef |grep nginx', 'BackDut', 'nginx: worker process')
        ...
        for key in self.case2_step2:
            re = fun.wait_data(self.case2_step2[key][0], 'FrontDut', self.case1_step2[key][1], '前置机配置', 100)
        ...
        status_code = http_check.http_get(url,flag=1)
        print('url:', url)
        print('定制应用命令为小写时,get请求的请求内容为：{}'.format(status_code))
        assert status_code == 200

        # 文件查看audit日志
        fun.wait_data(f"grep -n {proxy_port} /usr/local/nginx/logs/audit.log |tail -2", 'FrontDut',self.case2_step3['step1'][0], '检查小写字母cmdword get', 300, flag='存在')
    ...
    # @pytest.mark.skip(reseason="skip")
    @allure.feature('验证过滤类型为命令时，测试配置命令为其他字符时的定制应用通信情况')
    def test_customapp_order_other_character(self):

        # 下发配置
        fun.send(rbmExc, message.add_app_order_other_character_front['AddCustomAppPolicy'], FrontDomain, base_path)
        fun.send(rbmExc, message.add_app_order_other_character_back['AddCustomAppPolicy'], BackDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        front_res1 = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process', name='前置机nginx进程')
        assert front_res1 == 1
        fun.wait_data('ps -ef |grep nginx', 'BackDut', 'nginx: worker process')
        ...
        status_code = http_check.http_get(url,flag=1)
        print('url:', url)
        print('验证过滤类型为命令时，测试配置命令为其他字符时的定制应用通信情况,get请求的请求内容为：{}'.format(status_code))
        assert status_code == 200

        # 文件查看audit日志
        fun.wait_data(f"grep -n {proxy_port} /usr/local/nginx/logs/audit.log |tail -2", 'FrontDut',self.case3_step3['step1'][0], '检查其他字符cmdword :', 300, flag='存在')
    ...
    # @pytest.mark.skip(reseason="skip")
    @allure.feature('验证过滤类型为命令时，测试多个参数组合时的定制应用通信情况')
    def test_customapp_order_group(self):

        # 下发配置
        fun.send(rbmExc, message.add_app_order_group_front['AddCustomAppPolicy'], FrontDomain, base_path)
        fun.send(rbmExc, message.add_app_order_group_back['AddCustomAppPolicy'], BackDomain, base_path)
        fun.wait_data('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process')
        front_res1 = fun.nginx_worker('ps -ef |grep nginx', 'FrontDut', 'nginx: worker process', name='前置机nginx进程')
        assert front_res1 == 1
        fun.wait_data('ps -ef |grep nginx', 'BackDut', 'nginx: worker process')
        ...
        for key in self.case4_step1:
            ...
            print('app_order_group为：',self.case4_step1[key][1])
        ...
        # 文件查看audit日志
        fun.wait_data(f"grep -n {proxy_port} /usr/local/nginx/logs/audit.log |tail -2", 'FrontDut',self.case4_step3['step1'][0], '检查组合参数cmdword get', 300, flag='存在')

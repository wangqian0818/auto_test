...
print(base_path)
try:
    from vlan_connect import index
    from vlan_connect import message
...
class Test_vlan_connect():
    ...
    @allure.feature('验证supervlan与subvlan之间存在三层通信，subvlan之间存在二层隔离')
    def test_vlan_connect_a1(self):
        ...
        # 测试从vlanB ping设备正常
        dutB_cmd = 'ping ' + gwVlanIp + ' -c 4'
        print('从vlanB ping设备的命令是：{}'.format(dutB_cmd))
        dutB = fun.cmd(dutB_cmd, 'vlanB')
        assert '0% packet loss' in dutB

        # 测试vlanA与vlanB之间不可通信
        resAB_cmd = 'ping ' + vlanBOpeIp + ' -c 4'
        print('主机A与主机B之间测试通信的命令是：{}'.format(resAB_cmd))
        resAB = fun.cmd(resAB_cmd, 'vlanA')
        assert '100% packet loss' in resAB
    ...
    # @pytest.mark.skip(reseason="skip")
    @allure.feature(' 测试supervlan下的各种协议报文的连通性测试')
    def test_vlan_connect_a2(self):
        ...
        # 验证从vlanA发送scp命令上传
        scp_cmd = f'sshpass -p {gwPwd} scp /opt/pkt/vlanA.pdf root@{gwVlanIp}:/opt/pkt/'
        ...
        vlanB_url = 'wget -P /opt/pkt/ http://' + gwVlanIp + ':8889/gw.txt'

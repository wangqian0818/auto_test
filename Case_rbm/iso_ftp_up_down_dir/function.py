...
print(base_path)
try:
	from iso_ftp_up_down_dir import index
	from iso_ftp_up_down_dir import message
...
class Test_iso_ftp_up_down_dir():
    ...
    def test_iso_ftp_up_down_dir_a2(self):
        ...
        fp = con_ftp.connect_ftp(proxy_ip, self.ftp_proxy_port, self.ftp_user, self.ftp_pass)
        print('欢迎语是：{}'.format(fp.getwelcome()))
        result = con_ftp.uploadFileAll(fp, ftp_localDir)
        print('ftp走隔离上传一个文件夹内的所有文件和文件夹结果为:{}'.format(result))
        assert result == 1

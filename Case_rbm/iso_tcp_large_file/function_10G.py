...
class Test_iso_tcp_large_file():
    ...
    '''
    passed:可以下载成功，只是耗时比较久
    '''
    @pytest.mark.skip(reseason="skip")
    @allure.feature('验证隔离下的tcp策略（http）下载一个10G大小的文件')
    ...
    '''
    环境初始化：服务端添加10G.txt   
    cd /opt/pkt && dd if=/dev/zero of=10G.txt bs=1M count=10000
    ...
    def test_iso_tcp_large_file_a2(self):
        ...
        # # 在server的/opt/pkt/路径下创建一个10G大小的文件10G.txt
        ...
        assert '10G.txt' in touch_file

        # 验证隔离下的tcp策略（ssh、scp）从客户端发送scp命令下载
        scp_cmd = f'sshpass -p {BG8010ServerPwd} scp -P {ssh_proxy_port} root@{proxy_ip}:/opt/pkt/10G.txt /opt/pkt/'
        ...
        fun.cmd('rm -f /opt/pkt/10G.txt ', 'BG8010Client')
        check_file = fun.search('/opt/pkt', 'txt', 'BG8010Client')
        print('还原环境，10G.txt文件应该不在列表内，列表为{}'.format(check_file))
    ...
    '''
    环境初始化：客户端添加10G.pdf   
    cd /opt/pkt && dd if=/dev/zero of=10G.pdf bs=1M count=10000
    failed：sshpass上传超时，大文件需要上传很久
    '''
    @pytest.mark.skip(reseason="skip")
    ...
    def test_iso_tcp_large_file_a3(self):
        ...
        # # 在客户端的/opt/pkt/路径下创建一个10G大小的文件10G.pdf
        # touch_file_cmd = ['cd /opt/pkt', 'dd if=/dev/zero of=10G.pdf bs=1M count=10000']
        ...
        assert '10G.pdf' in touch_file

        # 验证隔离下的tcp策略（ssh、scp）从客户端发送scp命令上传
        scp_cmd = f'sshpass -p {BG8010ServerPwd} scp -P {ssh_proxy_port} /opt/pkt/10G.pdf root@{proxy_ip}:/opt/pkt/'
        ...
        fun.cmd('rm -f /opt/pkt/10G.pdf ', 'BG8010Server')
        check_file = fun.search('/opt/pkt', 'pdf', 'BG8010Server')
        print('还原环境，10G.pdf文件应该不在列表内，列表为{}'.format(check_file))

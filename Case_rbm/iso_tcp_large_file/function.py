...
class Test_iso_tcp_large_file():
    ...
    @allure.feature('验证隔离下的tcp策略（http）下载一个100M大小的文件')
    ...
    '''
    环境初始化：服务端添加100M.txt   
    cd /opt/pkt && dd if=/dev/zero of=100M.txt bs=1M count=100
    failed：sshpass下载超时，大文件需要下载很久
    '''
    ...
    def test_iso_tcp_large_file_a2(self):
        ...
        # 查询服务端的/opt/pkt 是否存在待下载的文件
        goal_file = fun.search('/opt/pkt', '100M.txt', 'BG8010Server')
        # print('goal_file:', goal_file)
        # 如果不存在，则新建文件
        if '100M.txt' not in goal_file:
            # 在server的/opt/pkt/路径下创建一个100M大小的文件100M.txt
            fun.cmd('cd /opt/pkt && dd if=/dev/zero of=100M.txt bs=1M count=100', 'BG8010Server')

        # 检查服务端文件是否创建成功
        touch_file = fun.search('/opt/pkt', 'txt', 'BG8010Server')
        print('检查服务端/opt/pkt/目录下所有以txt结尾的文件列表为：{}'.format(touch_file))
        assert '100M.txt' in touch_file

        # 验证隔离下的tcp策略（ssh、scp）从客户端发送scp命令下载
        scp_cmd = f'sshpass -p {BG8010ServerPwd} scp -P {ssh_proxy_port} root@{proxy_ip}:/opt/pkt/100M.txt /opt/pkt/'
        ...
        fun.cmd('rm -f /opt/pkt/100M.txt ', 'BG8010Client')
        check_file = fun.search('/opt/pkt', 'txt', 'BG8010Client')
        print('还原环境，100M.txt文件应该不在列表内，列表为{}'.format(check_file))
    ...
    '''
    环境初始化：客户端添加100M.pdf   
    cd /opt/pkt && dd if=/dev/zero of=100M.pdf bs=1M count=10000
    '''
    ...
    def test_iso_tcp_large_file_a3(self):
        ...
        # 查询客户端的/opt/pkt 是否存在待下载的文件
        goal_file = fun.search('/opt/pkt', '100M.pdf', 'BG8010Client')
        # print('goal_file:', goal_file)
        # 如果不存在，则新建文件
        if '100M.pdf' not in goal_file:
            # 在客户端的/opt/pkt/路径下创建一个100M大小的文件100M.pdf
            fun.cmd('cd /opt/pkt && dd if=/dev/zero of=100M.pdf bs=1M count=100', 'BG8010Client')

        # 检查客户端文件是否创建成功
        touch_file = fun.search('/opt/pkt', 'pdf', 'BG8010Client')
        print('检查客户端/opt/pkt/目录下所有以pdf结尾的文件列表为：{}'.format(touch_file))
        assert '100M.pdf' in touch_file

        # 验证隔离下的tcp策略（ssh、scp）从客户端发送scp命令上传
        scp_cmd = f'sshpass -p {BG8010ServerPwd} scp -P {ssh_proxy_port} /opt/pkt/100M.pdf root@{proxy_ip}:/opt/pkt/'
        ...
        fun.cmd('rm -f /opt/pkt/100M.pdf ', 'BG8010Server')
        check_file = fun.search('/opt/pkt', 'pdf', 'BG8010Server')
        print('还原环境，100M.pdf文件应该不在列表内，列表为{}'.format(check_file))
